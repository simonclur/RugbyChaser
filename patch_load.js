...
const replacement = `
        async function loadLiveMatchDetails(matchId, compName, venueName) {
            ...
            async function fetchDataAndUpdate() {
                try {
                    ...
                    targetContainer.innerHTML = \`<div class="spotlight"><p>Failed to load live tracking data.</p></div>\`;
                }
            }
...
const regex = /async function loadLiveMatchDetails\([^)]*\)\s*\{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}/;

// Wait, the regex fails linearly with many nested braces. 
// A better way is replacing from "async function loadLiveMatchDetails" until the line before "function renderMatches()"
